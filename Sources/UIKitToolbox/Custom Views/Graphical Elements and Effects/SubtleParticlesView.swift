//
//  Copyright © 2017 Apparata AB. All rights reserved.
//

#if canImport(UIKit)

import UIKit

public class SubtleParticlesView: UIView {

    private let emitterLayer = CAEmitterLayer()
    
    private let particleImage = UIImage(named: "particle", in: Bundle(for: SubtleParticlesView.self), compatibleWith: nil)

    public override init(frame: CGRect) {
        super.init(frame: frame)
        setUp()
    }
    
    public required init?(coder aDecoder: NSCoder) {
        super.init(coder: aDecoder)
        setUp()
    }
    
    override public func layoutSubviews() {
        let width = bounds.size.width
        let height = bounds.size.height
        emitterLayer.emitterPosition = CGPoint(x: width / 2.0, y: height / 2.0)
        emitterLayer.emitterSize = CGSize(width: width, height: height)
    }
    
    private func setUp() {
        let cell = CAEmitterCell()
        cell.birthRate = 1.0
        cell.lifetime = 50
        cell.velocity = 10
        cell.scale = 0.0
        cell.scaleRange = 0.0
        cell.scaleSpeed = 0.0085
        cell.alphaRange = 0.0
        cell.alphaSpeed = -1.0 / 4.0
        cell.color = UIColor.white.cgColor.copy(alpha: CGFloat(cell.lifetime * -cell.alphaSpeed))
        cell.emissionRange = CGFloat.pi * 2.0
        cell.contents = particleImage?.cgImage

        emitterLayer.emitterShape = CAEmitterLayerEmitterShape.rectangle
        emitterLayer.emitterCells = [cell]
        emitterLayer.opacity = 0.2
        
        layer.addSublayer(emitterLayer)
    }
}

public final class SubtleParticleImage {
    
    private static let imageBase64 = ""
    
    public static func make() -> UIImage {
        guard let data = Data(base64Encoded: imageBase64, options: .ignoreUnknownCharacters) else {
            return UIImage()
        }
        guard let image = UIImage(data: data) else {
            return UIImage()
        }
        return image
    }
}

#endif
